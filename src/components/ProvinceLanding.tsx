...
export function ProvinceLanding({ page }: { page: ProvincePage }) {
  const priceFrom =
    provinces.find((p) => p.code === page.code)?.priceFrom ?? 99;
  const activeCities = cityPages.filter((c) => c.provinceCode === page.code);
    ...
    <div className="theme-dark relative min-h-screen pt-24">
      ...
      <section className="border-y border-white/5 bg-white/[0.02] py-16">
        <Container>
          <SectionHeading
            ...
            title={`What ${page.name} duct systems are up against`}
          />
          <div className="mt-12 grid gap-6 md:grid-cols-3">
            {page.needs.map((point, i) => (
      ...
      <section className="border-t border-white/5 bg-white/[0.02] py-16">
        <Container>
          <SectionHeading
            eyebrow="FAQs"
            title={`${page.name} duct cleaning questions`}
          />
          <Reveal delay={0.1}>
            <ServiceFAQ
      ...
      <Section>
        <Container className="text-center">
          <h2 className="font-display text-3xl font-bold text-white sm:text-4xl">
            Book your {page.name} duct cleaning today
          </h2>
          <p className="mx-auto mt-4 max-w-xl text-ink-soft">
            Free quotes, upfront pricing from ${priceFrom}, and a 100%
